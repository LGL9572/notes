...
- 通过`git init`将这个文件夹变为git可管理的仓库

- 将项目粘贴到该文件夹下方，通过`git add .` 添加至仓库，并可以通过`git status`查看当前的状态

- 使用`git commit -m "描述文字"` 提交到仓库

- 关联仓库`git remote add origin git创建好的仓库地址`

- 提交项目远程仓库为空时需要加`-u` 这个参数`git push -u origin master`

  远程仓库不为空时则`git push origin master`

- 如果远程库存中有REAMDE文件则需要先拉取`git pull --rebase origin master `再push
